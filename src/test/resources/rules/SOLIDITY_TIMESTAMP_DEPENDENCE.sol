contract SolidityStyleGuideViolation {
    ...
    // <_yes> <report> SOLIDITY_TIMESTAMP_DEPENDENCE afb67a
        if (now > startTime + week) {
        g.h;
        }
    }
    function doSomething() {
        // <yes> <report> SOLIDITY_TIMESTAMP_DEPENDENCE 1955d9 <_yes> <report> SOLIDITY_TIMESTAMP_DEPENDENCE afb67a
        if ( startTime + week == block.timestamp) {}
        // <yes> <report> SOLIDITY_TIMESTAMP_DEPENDENCE 1955d9 <_yes> <report> SOLIDITY_TIMESTAMP_DEPENDENCE afb67a
        if ( startTime + week == now) {}
        if ( a == b) {}
        // <yes> <report> SOLIDITY_TIMESTAMP_DEPENDENCE cd1537 <_yes> <report> SOLIDITY_TIMESTAMP_DEPENDENCE afb67a
...
contract C {
    ...
    // <_yes> <report> SOLIDITY_TIMESTAMP_DEPENDENCE  afb67a <yes> <report> SOLIDITY_TIMESTAMP_DEPENDENCE cd1537
        d=now+1;
    }
